...
- `linear-auth.ts` — `resolveLinearAuthorization(inputs)` returns the `Authorization` header value: a raw personal API key (sent verbatim), or `Bearer <app token>` minted fresh per run via client credentials. Client credentials take precedence when both are set; throws `ERR_NO_LINEAR_AUTH` when neither is, and `ERR_PARTIAL_LINEAR_CLIENT_CREDENTIALS` when exactly one half of the client-credentials pair is set (an unset GitHub secret expands to `''`, so a half-configured pair must not fall through to the API key). Minted tokens are registered with `core.setSecret()` — the runner's log masker only knows values that arrived as `secrets.*`, so a runtime-minted token is unmasked until registered. `index.ts` wraps this in an **async** `linearFactory` (`run()` awaits the factory), so the token is only fetched when the Linked strategy actually reaches Linear.
